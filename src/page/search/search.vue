<template>
  <div class="paddingTop search_page">
    <hea-top head-title="搜索" goBack="true"></hea-top>
    <form class="search_form">
      <input type="search" name="search" class="search_input" v-model="searchValue" @input="checkInput" placeholder="请输入商家或美食名称">
    ...
    <section v-if="restaurantList.length">
      ...
      <ul class="list_container">
        <router-link v-for="item in restaurantList" :key="item.id" tag="li" class="list_li" :to="{path: '/shop', query: {id: item.id}}">
...
<script>
  ...
  export default {
    data () {
      return {
        restaurantList: [], // 存放搜索的商铺列表
        searchValue: '', // 搜索词
        geohash: '', // 存放位置
        searchHistory: [], // 搜索历史的展示
