        ...
        <hgroup class="shop_right">
          <header class="shop_detail_header">
            ...
            <ul class="shop_detail_ul">
              <li v-for="items in item.supports" :key="items.id" class="supports">{{items.icon_name}}</li>
            </ul>
          </header>
  ...
  export default {
    data () {
      return {
        ...
        shopListArr: [], // 店铺数据
        preventRepeatReuqest: false, // 请求状态
    ...
    methods: {
      ...
      async loadMore() {
        ...
        if (res.length < 20) { // 如果res的数据小于20条，说明数据已经不够了，不能再继续加载了
          this.touchend = true;
        }
      ...
      //返回顶部
      backTop(){
        animate(document.body, {scrollTop: '0'}, 400, 'ease-in');
      },
    },
    mounted() {
      this.initData();
